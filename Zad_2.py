...
# Dodatkowe metryki dla każdej klasy
ham_recall = cm[0][0] / (cm[0][0] + cm[0][1])
spam_recall = cm[1][1] / (cm[1][0] + cm[1][1])
ham_precision = cm[0][0] / (cm[0][0] + cm[0][1])
spam_precision = cm[1][1] / (cm[1][0] + cm[1][1])

print(f"HAM Recall: {ham_recall:.2f}")
print(f"SPAM Recall: {spam_recall:.2f}")
...
best_words_spam = ' '.join([' '.join(spam_dataset.loc[spam_dataset['Spam']==1, 'Lemmatized_Text'].values.flatten())])
best_wordcloud = WordCloud().generate(best_words_spam)
plt.figure(figsize=(12, 8))
plt.imshow(best_wordcloud)
plt.axis("off")
plt.title('Spam - najlepszy model')
plt.show()